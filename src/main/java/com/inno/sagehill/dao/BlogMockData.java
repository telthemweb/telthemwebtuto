...
public class BlogMockData {
    ...
    // create blog
    public Blog createBlog(int id, String title, String description) {
        Blog newBlog = new Blog(id, title, description);
        blogs.add(newBlog);
        return newBlog;
    }
    // update blog
    public Blog updateBlog(int id, String title, String description) {
        for(Blog b: blogs) {
            if(b.getId() == id) {
